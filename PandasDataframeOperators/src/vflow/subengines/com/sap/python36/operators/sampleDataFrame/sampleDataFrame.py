...
def process(msg):
    ...
    if not api.config.invariant_column or api.config.invariant_column.upper == 'NONE' :
        # sample of whole df
        if len(df.index) < api.config.sample_size :
        ...
        mean_col = sc_df.mean() 
        if mean_col == 0 :
            warning = 'Column: ' + s_col + ' has no values'
            raise ValueError(warning)
        sample_size_invariant = int(api.config.sample_size / mean_col)
        sample_size_invariant = 1 if sample_size_invariant == 0 else sample_size_invariant  # ensure minimum
        sc_df = sc_df.sample(n=sample_size_invariant,random_state=api.config.random_state).to_frame()
